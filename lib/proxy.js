...
module.exports = function DHTMLForTheNewMillennium(options, req, res, next) {
  ...
  var waybackCookie = req.cookies.wayback;
  ...
  if (waybackCookie) {
    ...
    doWaybackQuery(urlObj, options.date, res);
  }
};

function doWaybackQuery(urlObj, date, res) {
  // Get the uri of the nearest snapshot.
  var destURL = urlObj.host + urlObj.path;
  var checkURL = 'http://archive.org/wayback/available?url=' + destURL + '&timestamp=' + date;
  log(checkURL);
...
function cyberspace(waybackURL, urlObj, res) {
  ...
  request.get(waybackURL + urlObj.host + urlObj.path)
    .on('response', function(response){
      response.headers['set-cookie'] = 'wayback=' + waybackURL;
